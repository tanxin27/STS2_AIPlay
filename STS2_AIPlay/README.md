...
# STS2_AIPlay — LLM 驱动的杀戮尖塔 2 自动游玩代理
...
1. 从 [Releases](https://github.com/collinzrj/STS2_AIPlay/releases) 下载最新的 `STS2_AIPlay-vX.X.X.zip`
2. 将 zip 解压到 Mod 文件夹：
...
3. 确保 `mods/` 下有一个 `STS2_AIPlay/` 文件夹
4. 编辑 `STS2_AIPlay/` 文件夹中的 `llm_config.json` — 填入你的 API 密钥
...
构建会自动将 `STS2_AIPlay.dll` 复制到 Mod 文件夹：

**Windows:**
```
<Steam>/steamapps/common/Slay the Spire 2/mods/STS2_AIPlay/
```

**macOS:**
```
~/Library/Application Support/Steam/steamapps/common/Slay the Spire 2/mods/STS2_AIPlay/
```
...
编辑 Mod 文件夹中的 `llm_config.json`（`STS2_AIPlay.dll` 旁边）：
...
├── STS2_AIPlay.csproj              构建配置
├── STS2_AIPlay.json                Mod 清单
├── src/
...
Mod 在 `STS2_AIPlay.dll` 旁边写入以下文件（共享相同的会话时间戳）：
